...
public class UserRepository implements GBRepository<User, Long> {
    ...
    public Optional<User> update(Long id, User user) {
        ...
        User editUser = users.stream()
            .filter(u -> u.getId()                                             // метод переберает Ѕƒ и свер€ет
                     .equals(id))                                              // с 58 стр. -> id и user
        .findFirst().orElseThrow(() -> new RuntimeException("User no fond"));  // если не найдет, выведет.
        ...
        List<User> users = findAll();
            for(User user: users) {
                if(Objects.equals(user.getId(),id)) {
                    ...
                    System.out.println("список пользователей после удалени€: ");
                    for(User o: users){
                        System.out.println(o);
                    }
                    List<String> lines = new ArrayList<>();
                    for(User u: users) {
                        lines.add(mapper.toString());
                    }
                    operation.saveAll(lines);
                    break;
                }
